// Tetris clone 遊戲邏輯（可重用模組）
// 用法：new Tetris(boardCanvas, queueCanvas, holdCanvas).start()
// ES module 自動為嚴格模式，所以不需要 'use strict'。
...
// 重力：自動下落間隔(ms)。0 = 關閉（手動測試模式）。想測自動下落就調大，例如 500。
const DROP_SPEED = 0;

// 方塊定義：每個方塊 4 個旋轉狀態，用 16-bit 表示 4x4（1=實心、0=空）。順序 T I O S Z L J。
const PIECES = [
